            ...
            <div>
                ...
                <div class="p-3 border-2 border-blue-400 rounded-lg mt-3 min-h-[5rem]">
                    <v-alert color="#bfdbfe" theme="dark" icon="mdi-account" density="compact" class="my-1"
                        v-for="item in dataUnits" :key="item.usuario">
                        {{ item.usuario }}
                    </v-alert>
                </div>
            </div>
...
import { findAllGroupsUnitsApi, createGroupsUnitsApi } from '@/api/GroupUnitsService';
...
export default ({
    ...
    setup() {
        ...
        const loadData = async () => {
            ...
            findAllUnitsApi(store.state.codcuenta, store.state.empresa),
                findAllGroupsUnitsApi(store.state.codcuenta, store.state.codcliente)
            ])
            dataUnits.value = responseUsers.data.data
