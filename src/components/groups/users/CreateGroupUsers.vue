<template>
    ...
    <v-dialog v-model="dialog" width="600" @click:outside="cancelItem">
        <v-card>
            <v-toolbar>
                <span class="px-4 w-full text-center text-blue-400 font-bold title_views">Crear grupo de usuarios</span>
            </v-toolbar>
            <v-card-text>
                <v-col cols="12">
        ...
        watch(() => dialog.value, (newVal) => {
            if (newVal == true) {
                ...
                findAllClientsApi(store.state.codcuenta, store.state.empresa)
                    .then(response => {
                        dataClients.value = response.data.data ? response.data.data.filter(client => client.rol != "Administrador") : []
                        itemsAvailable.value = dataClients.value.map(user => ({ name: user.usuario }))
                    })
            }
        })
