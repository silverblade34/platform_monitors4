<template>
    ...
    <v-dialog v-model="dialog" width="600" @click:outside="cancelItem">
        <v-card>
            <v-toolbar>
                <span class="px-4 w-full text-center text-blue-400 font-bold title_views">Crear grupo de unidades</span>
            </v-toolbar>
            <v-card-text>
                <v-col cols="12">
            ...
            if (newVal == true) {
                ...
                findAllUnitsApi(store.state.codcuenta, store.state.codcliente)
                    .then(response => {
                        ...
                        itemsAvailable.value = dataUnits.value.map(unit => ({ name: unit.placa }))
                    })
            }
        })
